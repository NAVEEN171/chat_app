...
const Contacts = ({
  ...
  useEffect(() => {
    if (search !== "" && contacts.length > 0) {
      ...
      let filteredarray = cons.filter((contact) =>
    ...
    const getMessages = async () => {
      ...
      if (messageArray.length > 0) {
        messageArray.sort((a, b) => {
          return (
            new Date(b.lastmessage.timestamps) -
            new Date(a.lastmessage.timestamps)
          );
        });
      }
      //console.log("arrays");
    ...
    const activestatus = async () => {
      ...
      promises = contactinfo.map(async (user) => {
        ...
        let data = await fetch(
          `${process.env.REACT_APP_DEPLOYMENT_BACKEND}/getuser/${user._id}`,
          {
            method: "GET",
            headers: {
              "Content-Type": "application/json",
  ...
  const clearmessages = (index) => {
    ...
    if (
      presentcontacts.length > 0 &&
      presentcontacts[index].unseenmessages > 0
    ) {
    ...
    <div className="contacts-wrapper" id="contactswrapper">
      ...
      {presentcontacts.length > 0 && (
        <div className="allusers">
          {presentcontacts &&
            presentcontacts.map((user, index) => {
              let dateObject = null;
              let hours = null;
              let minutes = null;
              ...
              if (
                presentcontacts.length > 0 &&
                presentcontacts[index]?.lastmessage?.timestamps
              ) {
                dateObject = new Date(
                  presentcontacts[index].lastmessage.timestamps
                );
              ...
              return (
                ...
                >
                  <div
                    className={
                      user._id === prevtouched
                        ? "currentholdwrapper touched"
                        : "currentholdwrapper"
                    }
                  >
                    <div className="img-wrapper">
                      ...
                      <div
                        className={
                          ...
                          presentstatus[index]?.isactive === true
                            ? "active"
                            : "inactive"
                        }
                      ></div>
                    ...
                    <div className="textwrapper">
                      ...
                      <div className="lastmsg">
                        <div className="first">
                          {presentcontacts.length > 0
                            ? presentcontacts[index]?.lastmessage?.txt ||
                              "say hello👋"
                            : "say hello👋"}
                        </div>
                        {presentcontacts.length > 0 &&
                        presentcontacts[index]?.unseenmessages > 0 ? (
                          <div className="last">
                            {presentcontacts[index]?.unseenmessages}
                          </div>
                        ) : null}
                      </div>
                    </div>

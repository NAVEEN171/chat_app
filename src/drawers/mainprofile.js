...
const Mainprofile = ({
  ...
  const changenamehandler = async () => {
    ...
    if (enteredname.length > 0) {
      ...
      let data = await fetch(
        `${process.env.REACT_APP_DEPLOYMENT_BACKEND}/changename/${currentholder._id}`,
        {
          method: "POST",
          headers: {
  ...
  const deletehandler = async () => {
    ...
    if (local._id) {
      let data = await fetch(
        `${process.env.REACT_APP_DEPLOYMENT_BACKEND}/delete/${local._id}`,
        {
          method: "DELETE",
          headers: {
            "Content-Type": "application/json",
    ...
    <div className="mainprofilewrapper" id="mainprofilewrapper">
      <div className="mainprofile">
        ...
        <div className="detailwrappers">
          ...
          <div className="changephotobutt">{currentholder.email}</div>
          <button
          ...
          <button className="deleteaccbutton" onClick={logouthandler}>
            Logout
          </button>

          <button className="deleteaccbutton" onClick={deletehandler}>
            Delete Account
          </button>
        </div>
      </div>
